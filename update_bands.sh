...
# Refreshes bands.txt from Wikidata: musical groups tagged with a rock/metal/
# punk genre, ranked by sitelink count (a decent cross-language popularity
# proxy), CC0-licensed so there's no attribution/ToS concern. Meant to run
# occasionally (e.g. daily via cron), NOT on every post_song_quote.sh run -
# Wikidata's public endpoint asks for a descriptive User-Agent and isn't meant
# for frequent hammering.
